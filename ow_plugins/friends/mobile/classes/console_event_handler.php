        ...
        else
        {
            switch ( $dto->getStatus() )
            {
                case FRIENDS_BOL_Service::STATUS_ACTIVE:
                    ...
                    if(isset($iisSecuritymanagerEvent->getData()['code'])){
                        $code = $iisSecuritymanagerEvent->getData()['code'];
                        $href = "javascript:if(confirm('".OW::getLanguage()->text('admin','are_you_sure')."')){location.href='".$router->urlFor('FRIENDS_MCTRL_Action', 'cancel', array('id' => $userId, 'redirect'=>true,'code'=>$code))."';}";
                    }
                    break;

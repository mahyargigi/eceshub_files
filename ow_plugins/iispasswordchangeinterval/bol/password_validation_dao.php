...
class IISPASSWORDCHANGEINTERVAL_BOL_PasswordValidationDao extends OW_BaseDao
{
     ...
     * @return IISPASSWORDCHANGEINTERVAL_BOL_PasswordValidation
     */
    public function createPasswordValidationObject($email, $username, $valid, $token, $time = null)
    {
        if($time==null){
            $time = time();
        }
     ...
     * @return IISPASSWORDCHANGEINTERVAL_BOL_PasswordValidation
     */
    public function updateTimePasswordChanged($time = null)
    {
        $passwordValidation = $this->getCurrentUser();
        if ($passwordValidation == null) {
            $passwordValidation = $this->createPasswordValidationObject(OW::getUser()->getEmail(), OW::getUser()->getUserObject()->username, true, null, $time);
        } else {
            $passwordValidation->setPasswordtime(time());
            $passwordValidation->setValid(true);
            $passwordValidation->setToken(null);

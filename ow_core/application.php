...
class OW_Application
{
    ...
    public function init()
    {
        ...
        $viewRenderer->assignVar('isAuthenticated', OW::getUser()->isAuthenticated());
         $viewRenderer->assignVar('bottomPoweredByLink', '<a href="https://www.shub.ir/" target="_blank" title="'.OW::getLanguage()->text('base','powered_by_community').'"><img src="' . $currentThemeImagesDir . 'powered-by-motoshub.png" /></a>');
...
//                $loginCookie = BOL_UserService::getInstance()->findLoginCookieByUserId($id);
                BOL_UserService::getInstance()->updateLoginCookie(trim($_COOKIE['ow_login']), (time() + 86400 * 7));
                setcookie('ow_login', trim($_COOKIE['ow_login']), (time() + 86400 * 7), '/', null, false, true);
            }
        }
    }

...
class Form
{
    ...
    public function __construct( $name )
    {
        ...
        $event = OW::getEventManager()->trigger(new OW_Event('iissecurityessentials.before.form.creation'));
    ...
    public function addElement( $element )
    {
        ...
        $event = OW::getEventManager()->trigger(new OW_Event(IISEventManager::DISTINGUISH_REQUIRED_FIELD, array('element' => $element)));
    ...
    public function isValid( $data )
    {
        ...
        foreach ( $this->elements as $element )
        {
            ...
            if($yearChangedEvent->getData() && isset($yearChangedEvent->getData()['from']) && isset($yearChangedEvent->getData()['to']))
            {
                $element->setValue(array('from' => $yearChangedEvent->getData()['from'], 'to' =>$yearChangedEvent->getData()['to'] ));
            }
            if ( !$element->isValid() )
            {
                $valid = false;
    ...
    {
        if($this->CSRF_hash_needed && BOL_PluginService::getInstance()->findPluginByKey('iissecurityessentials')!= null && OW::getPluginManager()->getPlugin('iissecurityessentials')->isActive()) {
            $csrf_hash_el_val = IISSECURITYESSENTIALS_BOL_Service::getInstance()->refresh_csrf_hash($this->elements, $this->getAction());

...
class IISLanguageUpdater
{
    ...
    public static function updateBaseOfEnLanguageValues($langEnId, $languageService)
    {
        ...
        $languageService->addOrUpdateValue($langEnId, 'base', 'questions_question_securityCode_label', 'Security Code');
        $languageService->addOrUpdateValue($langEnId, 'base', 'questions_question_presentation_securityCode_label', 'Security Code');
        $languageService->addOrUpdateValue($langEnId, 'base', 'questions_question_securityCode_description', 'Security Code is neccessary when you forgot your password and can not access your email');
        $languageService->addOrUpdateValue($langEnId, 'admin', 'questions_possible_values_description', 'Add up to 64 values.');
        $languageService->addOrUpdateValue($langEnId, 'admin', 'user_active_status', 'active');
        $languageService->addOrUpdateValue($langEnId, 'admin', 'no_plugins_for_manual_updates', 'No plugins for manual update');
		...
		$languageService->addOrUpdateValue($langEnId, 'base', 'user_list_type_online', 'Online');
		$languageService->addOrUpdateValue($langEnId, 'base', 'user_list_type_featured', 'Featured');
		$languageService->addOrUpdateValue($langEnId, 'base', 'user_list_type_birthdays', 'Birthdays');
		$languageService->addOrUpdateValue($langEnId, 'base', 'form_validator_wyswyg_error_message', 'Input should contain text');
        $languageService->addOrUpdateValue($langEnId, 'base', 'text_copied_to_clipboard', 'Text has been copied to clipboard');
        ...
        $languageService->addOrUpdateValue($langEnId, 'base', 'forgot_password_submit_label', 'Send');
    ...
    {
        $languageService->addOrUpdateValue($langFaId, 'base', 'questions_question_securityCode_label', 'کد امنیتی');
        $languageService->addOrUpdateValue($langFaId, 'base', 'questions_question_presentation_securityCode_label', 'کد امنیتی');
        $languageService->addOrUpdateValue($langFaId, 'base', 'questions_question_securityCode_description', 'در صورت فراموشی گذرواژه و عدم دسترسی به رایانامه٬ اعلام کد امنیتی به مدیریت سامانه ضروری است');
        $languageService->addOrUpdateValue($langFaId, 'admin', 'questions_possible_values_description', 'شما می‌توانید تا 64 مقدار بیفزایید');
        $languageService->addOrUpdateValue($langFaId, 'base', 'is_friend', 'مخاطب');
        $languageService->addOrUpdateValue($langFaId, 'admin', 'no_plugins_for_manual_updates', 'افزونه‌ای برای به‌روزرسانی دستی یافت نشد.');
        $languageService->addOrUpdateValue($langFaId, 'base', 'avatar_user_select_empty_list_message', 'برای ادامه دادن لازم است یک یا چند کاربر را انتخاب کنید');
        $languageService->addOrUpdateValue($langFaId, 'base', 'rate_cmp_auth_error_message', 'مهمانان اجازه امتیازدهی ندارند');
        $languageService->addOrUpdateValue($langFaId, 'admin', 'found_users', 'تعداد کاربران: {$count} نفر');
        ...
        $languageService->addOrUpdateValue($langFaId, 'base', 'mail_template_admin_invite_user_content_html', '<p> سلام <br/> <p>  <br/> <a href="{$avatar}">{$name}</a> شما را دعوت کرده است تا به {$site_name}  بپیوندید. شما می توانید با استفاده از پیوند زیر وارد بخش عضویت شوید: <br /> <a href="{$url}">عضویت</a>');
    ...
    {$site_url}');
        ...
        $languageService->addOrUpdateValue($langFaId, 'admin', 'mail_template_admin_invite_user_content_html', '<p> سلام <br/> <p>  <br/> <a href="{$avatar}">{$name}</a> شما را دعوت کرده است تا به {$site_name}  بپیوندید. شما می توانید با استفاده از پیوند زیر وارد بخش عضویت شوید: <br /> <a href="{$url}">عضویت</a>');
...
');
		...
		$languageService->addOrUpdateValue($langFaId, 'admin', 'no_items', 'موردی موجود نیست');
		...
		$languageService->addOrUpdateValue($langFaId, 'base', 'total_score_label', 'تعداد امتیازها:<span class="ow_outline">{$ratesCount}</span>');
...
اگر شما برای این کار هیچ اقدامی نکردید می‌توانید به راحتی این رایانامه را نادیده بگیرید و پست الکتریکی شما استفاده نخواهد شد');
        ...
        $languageService->addOrUpdateValue($langFaId, 'base', 'user_list_type_online', 'حاضرین');
        $languageService->addOrUpdateValue($langFaId, 'base', 'user_list_type_featured', 'برجسته');
        $languageService->addOrUpdateValue($langFaId, 'base', 'user_list_type_birthdays', 'روزهای تولد');
        $languageService->addOrUpdateValue($langFaId, 'base', 'form_validator_wyswyg_error_message', 'ورودی باید شامل متن باشد');
        $languageService->addOrUpdateValue($langFaId, 'base', 'text_copied_to_clipboard', 'متن کپی شد');
        ...
        $languageService->addOrUpdateValue($langFaId, 'base', 'mobile_notifications_console_empty', 'موردی وجود ندارد');
        $languageService->addOrUpdateValue($langFaId, 'base', 'preference_no_items', 'موردی وجود ندارد');

        $languageService->addOrUpdateValue($langFaId, 'base', 'forgot_password_submit_label', 'ارسال');
    }
...
آخرین فعالیت‌های شما در {$site_name} که قصد اطلاع از وضعیت آن‌ها را دارید:');
        ...
        if (IISSecurityProvider::existPluginKeyInActivePlugins($activePlugins, 'iissecurityessentials')) {
            ...
            $languageService->addOrUpdateValue($langFaId, 'iissecurityessentials', 'questions_section_password','گذرواژه');
            $languageService->addOrUpdateValue($langFaId, 'iissecurityessentials', 'password','گذرواژه');
        }

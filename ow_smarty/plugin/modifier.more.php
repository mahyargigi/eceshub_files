...
function smarty_modifier_more( $string, $length )
{
    ...
    if(mb_strpos($contentPart1,'<')!==false){
        $new_length = $length;
        $lastStarterTag = mb_strrpos($contentPart1,'<');
        $selfEnded = (mb_strpos($string, '>', $lastStarterTag)==mb_strpos($string, '/>', $lastStarterTag)+1);
        if($selfEnded) {
            $end2 = mb_strpos($string, '>', $lastStarterTag);
            if ($end2 + 1 > $length)
                $new_length = $end2 + 1;
        }
        else {
            $lastEnderTag = mb_strrpos($contentPart1, '</');
            if ($lastStarterTag !== $lastEnderTag) {
                $lastEnderTag = mb_strpos($string, '</', $lastStarterTag);
                $new_length = mb_strpos($string, '>', $lastEnderTag) + 1;
            } else {
                $end2 = mb_strpos($string, '>', $lastStarterTag);
                if ($end2 + 1 > $length)
                    $new_length = $end2 + 1;

    ...
    initForm: function( textareaId, submitId ){
        ...
        owForms[this.formName].bind('success',
            function(data){                
                ...
                var bg=button_elem.css('color').replace('rgba','rgb'); console.log(bg);
                var a=bg.slice(4).split(','); console.log(a);
                var newBg='rgba('+a[0]+','+parseInt(a[1])+','+parseInt(a[2])+',255)'; console.log(newBg);
       ...
       owForms[this.formName].bind('submit',
            function(data){
                ...
                var bg=button_elem.css('color').replace('rgba','rgb'); console.log(bg);
                var a=bg.slice(4).split(','); console.log(a);
                var newBg='rgba('+a[0]+','+parseInt(a[1])+','+parseInt(a[2])+',0)'; console.log(newBg);
                $(button_elem).css('color',newBg);
            }
        );
...
$(function() {
    ...
    function selectElementText(element) {
        ...
        el_co = $(element).css('color');
        el_bg = $(element).css('background-color');
    ...
    function clearSelection(){
        ...
        //$('.cb_selected').css('color', el_co);
        //$('.cb_selected').css('background-color', el_bg);
    }
        ...
        $( $selector ).each(function() {
            ...
            $(this).on( 'mousedown touchstart', function( e ) {
                ...
                timer = setTimeout(function(){
                    ...
                    $(element).before('<div><div class="cb_text_overlay"></div><div id="cb_menu"><div id="cb_close">‌ ‌ ‌X‌ ‌ ‌</div><div id="cb_copy">copy</div></div></div>');
                    var short_menu = $('#cb_menu');
                    //cb_close
                    $("#cb_close", short_menu).on( 'click', function( e ) {
                        $(short_menu).remove();
                        $('.cb_selected').removeClass('cb_selected');
                        $('.cb_text_overlay').addClass('cb_text_overlay_off');
                    });
                    ...
                    $("#cb_copy", short_menu).on( 'click', function( e ) {
                        //copyTextToClipboard(cb_text);
                        //OWM.info(OW.getLanguageText('base', 'text_copied_to_clipboard'));
                        clearSelection();
                    });
                },longpress);
            ...
            $('body').on("mouseup touchend",function(){
                if ( start>0 && new Date().getTime() >= ( start + longpress )  ) {

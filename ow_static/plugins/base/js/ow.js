var OwUtils = function(){
    ...
    this.bindAutoClicks = function(context){
        ...
        $.each(autoClicks, function(i,o){
            var context = $(o);
            if($('textarea.invitation, textarea[placeholder]', context).is(":focus")){
                $('.ow_submit_auto_click', context).show();
            }else {
...
OW_UsersApi = function( _settings )
{
    ...
    this.deleteUser = function(userId,code, callBack, showMessage)
    {
        ...
        floatBox = OW.ajaxFloatBox('BASE_CMP_DeleteUser', [{userId: userId,code: code, showMessage : showMessage && redirectUrl}],
        {
    ...
    this.blockUserWithConfirmation = function( userId, code, confirmationCallback, callback ) {
        var floatBox = OW.ajaxFloatBox("BASE_CMP_BlockUser", {userId: userId ,code:code}, {
            scope: {
                "confirmCallback": function() {
                    _usersApi.blockUser(userId, code, callback);
                    if ( $.isFunction(confirmationCallback) ) {
                        confirmationCallback.call(null, userId, code);

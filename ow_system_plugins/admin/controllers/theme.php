...
class ADMIN_CTRL_Theme extends ADMIN_CTRL_Abstract
{
    ...
    public function ajaxResponder()
    {
        ...
        if(isset($pluginIisSecurity) && $pluginIisSecurity->isActive() && $_POST['ajaxFunc']=='ajaxDeleteImage') {
            $code =$_GET['code'];
            if(!isset($code)){
    ...
    public function getPhotoList( $params )
    {
        ...
        if ( isset($_POST['date']) && !empty($_POST['date']) )
        {
            ...
            $isJalali = false;
            $lbChangeEvent = OW::getEventManager()->trigger(new OW_Event(IISEventManager::ON_AFTER_DEFAULT_DATE_VALUE_SET, array('changeNewsJalaliToGregorian' => true, 'faYear' =>  (int)$tmpDateArray[0], 'faMonth'=> (int)$tmpDateArray[1] ,'faDay'=> $faDay)));
            if($lbChangeEvent->getData() && isset($lbChangeEvent->getData()['gregorianYearNews'])){
                $year = $lbChangeEvent->getData()['gregorianYearNews'];
                $isJalali=true;
            }

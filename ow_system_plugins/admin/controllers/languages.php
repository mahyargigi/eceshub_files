...
class ADMIN_CTRL_Languages extends ADMIN_CTRL_Abstract
{
    ...
    public function index()
    {
        ...
        $currentSearch = isset($_GET['search'])?UTIL_HtmlTag::stripJs(UTIL_HtmlTag::stripTags($_GET['search'])):'';
        $this->assign('current_search', ( strlen($currentSearch) ) ? $currentSearch : 'Search..');
        ...
        foreach ($list as &$item){
            foreach ($item['data'] as &$data){
                $iisSecuritymanagerEvent= OW::getEventManager()->trigger(new OW_Event('iissecurityessentials.on.generate.request.manager',
                    array('senderId'=>Ow::getUser()->getId(),'receiverId'=>$receiverId,'isPermanent'=>true,'activityType'=>'delete_token_lang')));
                if(isset($iisSecuritymanagerEvent->getData()['code'])){
                    $data['code'] = $iisSecuritymanagerEvent->getData()['code'];
                }
            }
        }

...
final class BOL_UserService
{
    ...
    public function updatePassword( $userId, $password )
    {
        if ( !empty($password) )
        {
            ...
            OW::getEventManager()->trigger(new OW_Event(IISEventManager::ON_AFTER_PASSWORD_UPDATE,array('userId' => (int)$userId)));
        }
        else
        {
    ...
    public function sendAdminInvitation( $email )
    {
        ...
        $mail->setHtmlContent(OW::getLanguage()->text('base', 'mail_template_admin_invite_user_content_html', array('url' => $inviteUrl,'avatar'=>$inviterAvatarUrl,'name' =>$inviterName)));
        $mail->setTextContent(OW::getLanguage()->text('base', 'mail_template_admin_invite_user_content_text', array('url' => $inviteUrl,'avatar'=>$inviterAvatarUrl,'name' =>$inviterName)));
    ...
    public function getSignInForm( $formName = 'sign-in', $submitDecorator = 'button' )
    {
        ...
        OW::getEventManager()->trigger(new OW_Event(IISEventManager::ON_AFTER_SIGNIN_FORM_CREATED,array('form' => $form,)));
    ...
    public function processSignIn( $identity, $password, $rememberMe = false )
    {
        ...
        if ( $result->isValid() )
        {
            if ( $rememberMe )
            {
                $loginCookie = $this->saveLoginCookie(OW::getUser()->getId(), (time() + 86400 * 7));
    ...
    public function processResetPasswordForm( $data, BOL_User $user, $resetCode )
    {
        ...
        $resultOfEvenet = OW::getEventManager()->trigger(new OW_Event(IISEventManager::ON_PASSWORD_VALIDATION_IN_JOIN_FORM, array('value' => $data['password'])));

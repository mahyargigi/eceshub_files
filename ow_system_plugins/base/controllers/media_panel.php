    ...
    public function gallery( $params )
    {
        ...
        if ( OW::getRequest()->isPost() )
        {
            ...
            if($_POST['command']=='delete-image') {
                ...
                if(isset($img)) {
                    $filename = isset($img->getData()->filename)?$img->getData()->filename:$img->getData()->name;
                    $file_addr = OW::getPluginManager()->getPlugin('base')->getUserFilesDir() . $img->id . '-' . $filename;
                    @unlink($file_addr);
...
class UploadImageForm extends Form
{
    ...
    public static function addFile( $plugin )
    {
        ...
        $file_name = uniqid() . '.' . UTIL_File::getExtension($name);

        if(isset($_POST['title']) && strlen($_POST['title'])>0){
            $name = UTIL_HtmlTag::stripTags(UTIL_HtmlTag::stripJs( $_POST['title']));
        }
        ...
        OW::getStorage()->copyFile($tmpname, $uploaddir . $id . '-' . $file_name);
        @unlink($tmpname);
